...
<body>
    ...
    <form action="index.php" method="post">
        ...
        <select name="drug" id="drug">
            ...
            $conn = new mysqli('localhost', 'root', '', 'drug_dispenser');
            if ($conn->connect_error) {
                die("Connection failed: " . $conn->connect_error);
            }

            // Retrieve drugs from the 'drugs' table and populate the dropdown list
            $sql = "SELECT DrugName FROM drugs";
            $result = $conn->query($sql);
            while ($row = $result->fetch_assoc()) {
                echo '<option value="' . $row["DrugName"] . '">' . $row["DrugNname"] . '</option>';
            }
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (isset($_POST["status"])) {
            ...
            $insertStatus = "INSERT INTO prescriptions (status) VALUES ('$selectedStatus')";
